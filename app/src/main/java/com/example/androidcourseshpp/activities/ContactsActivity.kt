...
class ContactsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        initContactsList(getContactItems())

    }

    private fun initContactsList(items: List<ContactItem>) {
    ...
    private fun getContactItems(): List<ContactItem> {
        ...
        for (contactId in 0..10) {
            items.add(
                ContactItem(
                    nameList[contactId % 5],
                    careerList[contactId % 5],
                    URLImageList[contactId % 5]
                )
            )
        }
