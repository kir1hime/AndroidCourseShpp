...
suspend fun <T> safeApiCall(
    ...
    apiCall: suspend () -> T
): Result<T, DataError.NetworkError> = withContext(dispatcher) {
    ...
    try {
        ...
        Result.Error(DataError.NetworkError.REQUEST_TIMEOUT_ERROR)
    } catch (e: IOException) {
        Log.e(tag, "NetworkError error: ${e.message}")
        Result.Error(DataError.NetworkError.CONNECTION_ERROR)
...
private fun handleHttpException(
    ...
    tag: String
): Result.Error<DataError.NetworkError> {
    ...
    return when (exception.code()) {
        ...
        404 -> Result.Error(DataError.NetworkError.NOT_FOUNDED_ERROR)
